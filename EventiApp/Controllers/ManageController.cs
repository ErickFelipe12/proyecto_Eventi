    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.AddPhoneSuccess ? "Se agregó su número de teléfono."
                : message == ManageMessageId.RemovePhoneSuccess ? "Su número de teléfono fue eliminado."
                : "";

            var model = validateUserAccount();
            return View(model);
        }

        private User validateUserAccount()
        {
            ViewBag.IdDocumentType = new SelectList(db.Documents.ToList(), "IdDocumentType", "DocumentType");
            var userEmail = User.Identity.GetUserName();

            var model = db.Users.Where(a => a.Email == userEmail).FirstOrDefault();

            if (model != null)
            {
                if (this.User.IsInRole(RolesEnum.Guest))
                {
                    var detailEm = new List<Employee>();
                    var res = db.Employees.Where(e => e.IdUser == model.IdUser).ToList();
                    if (res != null)
                        detailEm = res;

                    model.Employees = detailEm;
                }
                else
                {
                    var detailCl = new List<Client>();
                    var resClient = db.Clients.Where(e => e.IdUser == model.IdUser).ToList();
                    if (resClient != null)
                        detailCl = resClient;

                    model.Clients = detailCl;
                }
